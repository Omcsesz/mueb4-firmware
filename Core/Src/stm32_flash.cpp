...
void stm32_flash::reprogramPage(const std::array<uint8_t, pageSize>& Buff,
  ...
  volatile uint16_t* ptr =
      reinterpret_cast<uint16_t*>(flash_addr + (pageSize * page_num));
...
void stm32_flash::erasePage(const uint32_t page_num) {
  ...
  FLASH->AR = reinterpret_cast<uint32_t>(
      reinterpret_cast<uint32_t>(flash_addr) +
