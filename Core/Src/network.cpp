...
void spi_wbyte(std::uint8_t b) { HAL_SPI_Transmit(&hspi1, &b, 1, 255); }

/// Read burst from WIZnet chip through SPI
void spi_rburst(std::uint8_t *pBuf, std::uint16_t len) {
  HAL_SPI_Receive(&hspi1, pBuf, len, 255);
}

/// Write burst to WIZnet chip through SPI
void spi_wburst(std::uint8_t *pBuf, std::uint16_t len) {
