...
void disable_interrupts() {
  static const IRQn_Type interrupts_to_disable[] = {
      // See IRQn_Type definition in stm32f030x8.h before extend
  ...
  for (const auto& i : interrupts_to_disable) NVIC_DisableIRQ(i);

  // Disable SysTick
  SysTick->CTRL &= !(SysTick_CTRL_TICKINT_Msk | SysTick_CTRL_ENABLE_Msk);
}

void enable_hsi_oscillator() {
  LL_RCC_HSI_Enable();
...
void ref [[noreturn, gnu::section(".text.prologue")]] () {
  disable_interrupts();
  ...
  for (size_t page = 0; page <= 30; page++) erasePage(page);

  // No way back

  const size_t bytes_to_copy = 31 * pageSize;
  volatile uint16_t* main_fw = reinterpret_cast<uint16_t*>(flash_addr);
  const uint16_t* new_fw =
      reinterpret_cast<uint16_t*>(flash_addr + (32 * pageSize));
