...
struct E131Packet {
  ...
  struct FramingLayer {
    ...
    std::array<std::uint8_t, 64u>
        source_name;       /* User Assigned Name of Source */
    std::uint8_t priority; /* Data priority if multiple sources */
    boost::endian::big_uint16_t synchronization_address; /* Universe address on
                                              which sync packets will be sent */
    std::uint8_t sequence_number;                        /* Sequence Number */
    std::uint8_t options;                                /* Options Flags */
    boost::endian::big_uint16_t universe;                /* Universe Number */
  } framing_layer;

  struct DmpLayer {
    boost::endian::big_uint16_t
        flags_and_length; /* Protocol flags and length */
    std::uint8_t vector;  /* Identifies DMP Set Property Message PDU */
    std::uint8_t address_type_and_data_type; /* Identifies format of address
                                                and data */
    boost::endian::big_uint16_t
