...
#warning \
    "Az atollic StruStudio9 beepitett forditoja rossz bytesorrendet hasznal. a gcc-t rossz kapcsolokkal forditottak, nem lehet atrini... :/"
...
struct [[gnu::packed]] matrix2_package {
 private:
  ...
  uint8_t section;  // Tells which forth of the Schonherz is described in the
                    // package
 ...
 public:
  inline static const size_t roomNumber2roomIndex(const uint8_t floor,
                                                  const uint8_t roomnumber) {
  ...
  inline bool isValid() const { return validity_byte == 0x01; }

  inline const auto& getRoom(const size_t index) const { return rooms[index]; }

  inline const auto& getRoom(const uint8_t floor, const uint8_t roomnumber)
      const {
    return rooms[roomNumber2roomIndex(floor, roomnumber)];
  }

  inline const bool isMyZone(const uint8_t floor, const uint8_t roomnumber)
      const {
